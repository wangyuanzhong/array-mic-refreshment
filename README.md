...
**隐私文案** = 在托盘、关于页、首次启动里用**普通人能看懂的一句话**，说明数据怎么处理，减少用户担心「是不是在偷听上传」。

和本产品的真实行为对应关系：

| 实际行为 | 建议文案（示例） |
|----------|------------------|
| 音频、声纹 enrollment 均在本机 | 「麦克风数据与声纹模板仅保存在本机，不上传。」 |
| 按住快捷键才采集 | 「仅按住 {HotKey} 时录音，松开即停止。」 |
| 仅当前选中用户可通过门禁 | 「仅对已注册的「{当前用户}」声音进行识别。」 |
| Agent/API 会发文字出去 | 「识别后的文字会发送到您配置的 API 地址做整理，请自行信任该服务。」（**若启用后处理则必须写清楚**） |

**你需要做的决定：**

- 若 **Agent/API 默认关闭** 或仅本地：可写「默认不出网」。
- 若 **一定会调云端 API**：不能只写「均不出本机」——否则文案虚假。应拆成 **「音频不出网」** + **「整理后的文字会按设置调用 API」**。

**首版建议：**

- 关于页两段：**本地音频与声纹** / **可选云端文本整理**。
- 首次启用 API 时弹窗确认一次。
- 不强制在托盘长期占一行（可选「ℹ 隐私」菜单项）。
